...
public final class Board implements Runnable {
    ...
    public void run() {
        for (Player player : Bukkit.getOnlinePlayers()) {
            if (player.getScoreboard() != null && player.getScoreboard().getObjective(Essentials.getInstance().getName()) != null)
    ...
    private void createNewScoreboard(Player player) {
        ...
        Objective objective = scoreboard.registerNewObjective(Essentials.getInstance().getName(), "yummy");
        ...
        Objective objectiveHealth = scoreboard.registerNewObjective(
                Essentials.getInstance().getName() + "_health", Criterias.HEALTH);
